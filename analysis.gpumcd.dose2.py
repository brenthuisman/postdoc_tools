...
image.image(os.path.join(rundir,'gpumcd_dose_water2.mhd')), #dose to water, propagate water
image.image(os.path.join(rundir,'gpumcd_dose_water.mhd')), #dose to water, propagate medium
...
xsize = np.linspace( 0, int( images[0].header['ElementSpacing'][0]*images[0].header['DimSize'][0] ), images[0].header['DimSize'][0] )

f, (row1,row2,row3,row4) = plot.subplots(nrows=4, ncols=2, sharex=False, sharey=False)

for im,leg in zip( [images[0],images[3]], [names[0],names[3]] ):
    col=random.choice(plot.colors*3)
    row1[0].plot(xsize,im.getline('y'),alpha=0.5,color=col,label=leg)
    row1[1].plot(xsize,im.getline('z'),alpha=0.5,color=col,label=leg)
for im,leg in zip( [images[0],images[2]], [names[0],names[2]] ):
    col=random.choice(plot.colors)
    row2[0].plot(xsize,im.getline('y'),alpha=0.5,color=col,label=leg)
    row2[1].plot(xsize,im.getline('z'),alpha=0.5,color=col,label=leg)
for im,leg in zip( [images[0],images[1]], [names[0],names[1]] ):
    col=random.choice(plot.colors)
    row3[0].plot(xsize,im.getline('y'),alpha=0.5,color=col,label=leg)
    row3[1].plot(xsize,im.getline('z'),alpha=0.5,color=col,label=leg)
for im,leg in zip( images[1:], names[1:] ):
    col=random.choice(plot.colors)
    row4[0].plot(xsize,im.getline('y'),alpha=0.5,color=col,label=leg)
    row4[1].plot(xsize,im.getline('z'),alpha=0.5,color=col,label=leg)
